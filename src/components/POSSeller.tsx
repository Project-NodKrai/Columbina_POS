...
import { collection, onSnapshot, addDoc, doc, updateDoc, increment, serverTimestamp, query, orderBy, limit, getDocs, where, getDoc, runTransaction, deleteDoc } from 'firebase/firestore';
...
export function POSSeller() {
  ...
  const handleDeleteSession = async (sessionId: string) => {
    ...
    try {
      const path = `stores/${store.id}/businessSessions/${sessionId}`;
      ...
      if (errorMsg.includes('permission')) {
        setError(`삭제 권한이 없습니다. (Store: ${store.id}, Session: ${sessionId})`);
      } else {
        setError(`매출 리포트 삭제 중 오류가 발생했습니다: ${errorMsg}`);
      }
    } finally {
      setIsDeleting(false);
    }
        ...
        <div className="fixed inset-0 z-[130] flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-md">
          <div className="bg-white rounded-[32px] shadow-2xl w-full max-w-lg overflow-hidden animate-in fade-in zoom-in duration-200">
            <div className="p-8 border-b border-slate-100 flex justify-between items-center">
              ...
              <button onClick={closeSessionModal} className="p-2 hover:bg-slate-100 rounded-full transition-colors">
                <X className="w-6 h-6 text-slate-400" />
              </button>
            </div>
            ...
            <div className="p-8 space-y-8">
              ...
              <div className="flex gap-3">
                {deleteConfirm ? (
                  <div className="flex-1 flex flex-col gap-3 p-4 bg-red-50 rounded-2xl border border-red-100">
                    <p className="text-sm font-bold text-red-900 text-center">정말 삭제하시겠습니까?</p>
                    <div className="flex gap-2">
                      <button 
                        onClick={() => setDeleteConfirm(false)}
                        className="flex-1 py-2 bg-white text-slate-600 rounded-xl font-bold border border-slate-200 text-sm"
                      >
                        취소
                      </button>
                      <button 
                  ...
                  <>
                    ...
                    <button 
                      ...
                      className="px-6 py-4 bg-red-50 text-red-600 rounded-2xl font-bold hover:bg-red-100 transition-all flex items-center justify-center"
                      title="리포트 삭제"
                    >
